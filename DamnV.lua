...
function ESP:GetColor(obj)
	...
	return p and (self.UseTeamColor and p.Team and p.Team.TeamColor.Color) or
		(p.Team and p.Team.TeamColor ~= LocalPlayer.Team.TeamColor and self.EnemyColor or self.TeamColor) -- self.Color
end
					...
					local box =
						ESP:Add(
							c,
							{
								...
								Name = type(options.CustomName) == "function" and options.CustomName(c) or
								options.CustomName,
		...
		if Vis5 and Char:FindFirstChild("Humanoid") then
			...
			if ESP.Info.Health.Value == true then
				...
				self.Components.Health.Text =
					"[" ..
					math.floor(Char:FindFirstChildOfClass("Humanoid").Health) ..
					"/" .. math.floor(Char:FindFirstChildOfClass("Humanoid").MaxHealth) .. "]"
	...
	if ESP.Tracers.Value then
		...
		if Vis6 then
			...
			self.Components.Tracer.To =
				Vector2.new(CurrentCamera.ViewportSize.X / 2, CurrentCamera.ViewportSize.Y / ESP.AttachShift)
		...
		setmetatable(
			{
				...
				PrimaryPart = options.PrimaryPart or
				obj.ClassName == "Model" and
				(obj.PrimaryPart or obj:FindFirstChild("HumanoidRootPart") or obj:FindFirstChildWhichIsA("BasePart")) or
